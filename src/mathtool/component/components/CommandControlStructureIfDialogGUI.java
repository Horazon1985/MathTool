 ...
 */
public class CommandControlStructureIfDialogGUI extends AlgorithmCommandComponentTemplate {
    ...
    private static CommandControlStructureIfDialogGUI instance = null;

    private CommandControlStructureIfDialogGUI(int algorithmGuiX, int algorithmGuiY, int algorithmGuiWidth, int algorithmGuiHeigh) {
        ...
        this.generateButton.addActionListener((ActionEvent e) -> {
            try {
                algorithmEditor.getDocument().insertString(algorithmEditor.getCaretPosition(),
                        MathToolAlgorithmsController.generateControlStructureIf(textFields[0].getText()), null);
                dispose();
    ...
    public static CommandControlStructureIfDialogGUI createCommandControlStructureIfDialog(int algorithmGuiX, int algorithmGuiY, int algorithmGuiWidth, int algorithmGuiHeigh,
            JTextArea algEditor) {
        algorithmEditor = algEditor;
        if (instance == null) {
            instance = new CommandControlStructureIfDialogGUI(algorithmGuiX, algorithmGuiY, algorithmGuiWidth, algorithmGuiHeigh);
        }
